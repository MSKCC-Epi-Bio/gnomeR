...
```
    ...
    * Wide-format - Organized such that there is one column per sample and one row per gene. Thus, each sample's events are contained within one column. This is most common format you will receive when downloading data from the cBioPortal web browser.  
...
The `gene_binary` argument expects binary data as generated by the `create_gene_binary()` or `summarize_by_gene()` functions. Example below shows the summary using gene data for ten samples. 
...
- `freq_cutoff` argument subsets the list of genes to those with events observed in at least X% of the samples. In the example below for `tb1`, if the gene is altered in at least 15% of samples, the gene is included in the summary table.
...
tbl_genomic(gene_binary = gene_binary, gene_subset = c("PARP1", "ALK"))
...
There are a set of default pathways available in the package that can be viewed using `gnomeR::pathways`. This new data frame will include columns for mutations, CNAs, structural variants, and pathways. You can subset to only the pathways if you choose.
...
gnomeR comes with 66 different color options broken into 3 distinct palettes, pancan, main, and sunset. Typically pancan and main are discrete color palettes where sunset is a continuous color palette. You can see all colors with `gnomer_colors`, and the 3 distinct palettes with `gnomer_palettes$pancan`, `gnomer_palettes$main`, `gnomer_palettes$sunset`. 

### `gnomer_palette()`

`gnomer_palette()` is used to set/subset specific palettes, create a continuous palette, and/or plot palettes to show the user the specific colors they chose. 

The code below will pick the first 4 colors from the pancan palette for a discrete palette.
...
# you can store the palette as a vector and call the colors later 
example_pal <-  gnomer_palette("pancan")
```

### `set_gnomer_palette()`

If you do not want to constantly have to set the palette for each plot you can set a palette theme for your entire session and do not need to call `gnomer_palette()` every time. `set_gnomer_palette` allows you to choose from one of the 3 palettes in `gnomer_palettes`. You can set the discrete and gradient palette independently. With the examples below you can see the default colors change for each plot without having to call a `scale_*` function.
...
If you do not want to use the palettes from `set_gnomer_palette()` anymore you can reset the palettes back to ggplots color palettes with `reset_gnomer_palette()`. This function only has one argument and you will rarely have to change it. 
