...
- clin.dat : optional data frame of clinical covariates
...
Similarly we include here an example adding clinical variables:

```{r}
clin.dat <- clin[match(rownames(gen.dat),clin$DMPID),] %>% 
  select(DMPID, Sex,Smoker) %>% 
  filter(!is.na(DMPID))
gen.dat <- gen.dat[match(clin.dat$DMPID,rownames(gen.dat)),]
clin.dat <- clin.dat %>%
  tibble::column_to_rownames('DMPID')
plot_oncoPrint(gen.dat = gen.dat,clin.dat = clin.dat)
```
