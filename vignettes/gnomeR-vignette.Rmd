...
```{r}
outcome <- as.character(clin.patients$Sex[match(patients,clin.patients$DMPID)])
...
- gen.dat : data frame of binary genetic events as created by the `create.bin.matrix` function
...
```{r}
gen.dat <- bin.mut$mut[,names(sort(apply(bin.mut$mut,2, sum),decreasing = T))[1:15]]
plot_oncoPrint(gen.dat)
```


Similarly we include here an example adding clin.patientsical variables:

```{r}
clin.patients.dat <- clin.patients[match(rownames(gen.dat),clin.patients$DMPID),] %>% 
  select(DMPID, Sex,Smoker) %>% 
  filter(!is.na(DMPID))
gen.dat <- gen.dat[match(clin.patients.dat$DMPID,rownames(gen.dat)),]
clin.patients.dat <- clin.patients.dat %>%
  tibble::column_to_rownames('DMPID')
plot_oncoPrint(gen.dat = gen.dat,clin.patients.dat = clin.patients.dat)
```
...
- ordered : order in which patients should be printed in the heatmap
- outcome : potential outcome of interest to be plotted along the X-axis

```{r}
# facets.heatmap(seg = seg, patients=patients[50:100])
```
