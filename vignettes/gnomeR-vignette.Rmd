...
```{r}
patients <- as.character(unique(mut$Tumor_Sample_Barcode)) #as.character(unique(mut$Tumor_Sample_Barcode[1:10000]))
bin.mut <- create.bin.matrix(patients = patients,maf = mut,mut.type = "SOMATIC",SNP.only = F,include.silent = F, spe.plat = T)
as.tbl(bin.mut$mut)
```
...
```{r}
bin.mut <- create.bin.matrix(patients = patients,maf = mut,mut.type = "SOMATIC",SNP.only = F,include.silent = F, fusion = fusion, cna = cna, spe.plat = T)
as.tbl(bin.mut$mut)
```
...
surv.dat <- clin.patients %>%
  ...
  mutate(time = as.numeric(as.character(time)),
    ...
    filter(!is.na(time))
X <- bin.mut$mut[match(surv.dat$DMPID,abbreviate(rownames(bin.mut$mut),strict = T, minlength = 9)),]
...
```{r}
gen.dat <- bin.mut$mut[1:1000,names(sort(apply(bin.mut$mut,2, sum),decreasing = T))[1:15]]
