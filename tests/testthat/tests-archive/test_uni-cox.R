...
#   expect_warning(gen.dat <- binmat(samples = samples, maf = mut, cna = cna, cna.binary = TRUE))
...
#   expect_warning(gen.dat <- binmat(samples = samples, maf = mut, cna = cna, cna.binary = FALSE))
#   expect_warning(test <- uni.cox(X = gen.dat,surv.dat = surv.dat,surv.formula  = Surv(time,status)~.,filter = 0,genes = NULL))
